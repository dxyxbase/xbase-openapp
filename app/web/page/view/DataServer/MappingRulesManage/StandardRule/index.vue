<template>
  <div class="model-class">
    <div class="head-btn">
      ...
      <a-alert message="该规则适用于标准映射管理自动映射过程。自动映射将按规则顺序执行，有匹配结果则执行终止，无匹配结果则执行下一条规则，请注意规则顺序！" banner />
    </div>
    <div class="table-box">
    ...
    <a-modal destroyOnClose width="800px" centered v-model="visibleEdit" :title="type === 'add' ? '新建标准规则' : '编辑标准规则'" @cancel="closeModel" ok-text="确认" cancel-text="取消" @ok="sureEdit">
      <a-form :form="form">
        <div v-for="(item, index) in keysList" :key="item">
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :label="`映射规则${index + 1}`" :labelCol="{ span: 9 }" :wrapperCol="{ span: 15 }">
                <a-select
                  placeholder="分类标准参数"
                  v-decorator="[
                    `model_property[${item}]`,
                    {
                      initialValue: arr[item] ? arr[item].model_property : 1,
                      rules: [{ required: true, message: '请选择' }]
                    }
                  ]"
                >
                  <a-select-option v-for="(val, key) in model_property_obj" :key="key" :value="val.value">{{ val.name }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="4">
            ...
            <a-col :span="6">
              <a-form-item label="" :labelCol="{ span: 0 }" :wrapperCol="{ span: 24 }">
                <a-select
                  placeholder="映射标准参数"
                  v-decorator="[
                    `mapping_property[${item}]`,
                    {
                      initialValue: arr[item] ? arr[item].mapping_property : 1,
                      rules: [{ required: true, message: '请选择' }]
                    }
                  ]"
                >
                  <a-select-option v-for="(val, key) in mapping_property_obj" :key="key" :value="val.value">{{ val.name }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
...
<script>
import { rule_standard_list, rule_standard_add, rule_standard_move, rule_standard_reset, rule_standard_del, rule_standard_edit, rule_standard_check } from '@/apis/rules.js'
...
export default {
  data() {
    return {
      ...
      // 映射标准参数。1属性组名称，2属性名称
      mapping_property_obj: [
        { name: '属性组名称', value: 1 },
        { name: '属性名称', value: 2 }
      ],
      // 分类标准参数。1分类名称
      model_property_obj: [{ name: '分类名称', value: 1 }]
    }
  },
  created() {
  ...
  methods: {
      ...
      validateFields(async (errors, values) => {
        if (!errors) {
          ...
          for (let i = 0; i < numer; i++) {
            arr.push({
              model_property: parseInt(values.model_property[i]),
              operator: parseInt(values.operator[i]),
              mapping_property: parseInt(values.mapping_property[i])
            })
          }
          let params = {
          ...
          let res = null
          if (this.type === 'add') {
            res = await rule_standard_add(params)
          } else {
            res = await rule_standard_edit({ ...params, rule_id: this.rule_id })
          }
          this.arr = []
          this.joint = 1
    ...
    async check() {
      const result = await rule_standard_check()
    ...
    async move(type, item) {
      ...
      const result = await rule_standard_move(params)
      if (result.code !== ResponseStatus.success) return
      this.$message.success(type === 1 ? '上移成功' : '下移成功')
      this.getList()
    },
    async resetRule() {
      const result = await rule_standard_reset()
      if (result.code !== ResponseStatus.success) return
    ...
    del(item) {
      this.$confirm({
        ...
        onOk: () => {
          ...
          rule_standard_del(params).then(res => {
    ...
    async getList() {
      let params = {}
      const res = await rule_standard_list(params)
      if (res.code !== ResponseStatus.success) return
      const {
        data: { list = [] }
      } = res
